...
void mazerunner::sensorCallback(const ras_arduino_msgs::ADConverter msg){
...
mazerunner::mazerunner(int argc, char *argv[]){
	ros::init(argc, argv, "mazerunner");	//name of node
	...
	sub_sensor = handle.subscribe("/arduino/adc", 1000, &mazerunner::sensorCallback, this);
...
{
    mazerunner mazerunner(argc, argv);
}
